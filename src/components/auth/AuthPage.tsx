...
import { Mail, Lock, Github, AlertCircle, Eye, EyeOff } from "lucide-react";
...
const AuthPage = ({ onAuthSuccess }: AuthPageProps) => {
  ...
  const handleEmailAuth = async (type: 'signin' | 'signup') => {
    ...
    try {
      ...
      } else if (type === 'signup' && !result.data.session) {
        toast({
  ...
  const handleBitbucketAuth = async () => {
    ...
    try {
      ...
      setError('Failed to sign in with Bitbucket');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl font-bold text-foreground">FixMate AI</h1>
          <p className="text-muted-foreground">Automated bug fixing platform</p>
        </div>
